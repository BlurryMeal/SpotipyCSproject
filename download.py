...
client_id = '63c65f9460d94484a147d52ae11078c7'  #spotify keys
...
class SpotifyAPI(object):
    ...
    def playlist_name(self, link):
        ...
        endpoint = 'https://api.spotify.com/v1/playlists/'
        lookup_url = f"{endpoint}{target_URI}"
        r = requests.get(lookup_url, headers=headers)
        if r.status_code not in range(200, 299):
            return {}
        return r.json()
...
# async def downloader(directory, song):
...
#
#
# async def queue_all(directory, songs):
#     downloaded = []
#     for song in songs:
#         downloaded.append(downloader(directory, song))
#     print(downloaded)
#
#     await asyncio.gather(*downloaded)
#
#
spotify = SpotifyAPI(client_id, client_secret)


def download(name, email, link, length_req):
    user = {
        'name': name,
        'email': email,
        'link': link,
    ...
    playlist_name = "".join(x for x in spotify.playlist_name(user['link'])['name'] if (x.isalnum() or x in "._- "))
    ...
    # asyncio.run(queue_all(directory, songs))
    base = 'https://www.youtube.com'
    for song in songs:
    ...
    with ZipFile(f"{playlist_name}.zip", 'w') as zip:
        for file in file_paths:
            zip.write(file)

    file = drive.CreateFile(
        {
            'title': f"{playlist_name}.zip",
            'parents': [{'kind': 'drive#fileLink',
                         'id': "1sceeoXB0jRDm89GQF-yJQtbOz41gEjgu"}]
        }
    )
    file.SetContentFile(f"{playlist_name}.zip")
    file.Upload()
    file_id = file['id']
    ...
    message['from'] = 'sagarg2606@gmail.com'
...
"""
    ...
    password = "Sagargupta2606"
    server = smtplib.SMTP('smtp.gmail.com:587')
    server.ehlo()
    server.starttls()
    server.login('purpadri.uiux@gmail.com', password)
    server.sendmail('purpadri.uiux@gmail.com', user['email'].lower(), message.as_string())
    server.quit()
    print("Link mailed")
    try:
...
if __name__ == '__main__':
    for i in queue:
        ...
        email = i['email']
        link = i['link']
        length_req = i['length_req']
        first = Process(target=download, args=(name, email, link, length_req)).start()
